...
def camera_1():
  ...
  if plate_1.match(plate) or plate_2.match(plate) or plate_3.match(plate) or plate_4.match(plate) or plate_5.match(plate) or plate_6.match(plate) or plate_7.match(plate):
    if rdb.execute("select (1) from plates where plate = '"+plate+"' limit 1;"): 
      return '{"error":"False","stored":"True"}'+plate
    else:
      return '{"error":"False","stored":"False1"}'+plate
  else:
